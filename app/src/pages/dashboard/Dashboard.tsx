      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {kpis.map((k) => (
          <Card key={k.label} className="p-5">
            ...
            {k.delta && (
              <div className="mt-1 text-xs text-[hsl(var(--brand))]">{k.delta}</div>
            )}
          </Card>
        ))}
      </section>
                ...
                <AreaChart data={donationsMonthly} margin={{ left: -10, right: 10, top: 10 }}>
                  <defs>
                    <linearGradient id="g1" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={`hsl(var(--chart-fill))`} stopOpacity={0.5}/>
                      <stop offset="95%" stopColor={`hsl(var(--chart-fill))`} stopOpacity={0}/>
                    </linearGradient>
                  </defs>
                  <CartesianGrid stroke="var(--chart-grid)" vertical={false} />
                  <XAxis dataKey="m" tickMargin={8} tick={{ fill: "var(--chart-tick)", fontSize: 12 }} />
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stockByType} margin={{ left: -10, right: 10, top: 10 }}>
                  <CartesianGrid stroke="var(--chart-grid)" vertical={false} />
                  <XAxis dataKey="name" tickMargin={8} tick={{ fill: "var(--chart-tick)", fontSize: 12 }} />
                  <YAxis tick={{ fill: "var(--chart-tick)", fontSize: 12 }} />
                  <Tooltip
                    contentStyle={{
                      borderRadius: 12,
                  ...
                  />
                  <Bar dataKey="qty" fill={`hsl(var(--brand))`} radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardBody>
