...
export default function DeliveryDetailPage() {
  ...
  // ---- carregar os lotes referenciados (para mostrar nome/código) ----
  ...
  useEffect(() => {
    ...
    (async () => {
      // json-server aceita /stockLots/:id; caso uses outro nome, ajusta o endpoint aqui
      const results = await Promise.all(
        lotIds.map(async (lotId) => {
          try {
            ...
            // fallback: /stockLots?id=xxx
            const resList = await api.get<StockLot[]>("/stockLots", {
              params: { id: lotId },
            });
            return Array.isArray(resList.data) ? resList.data[0] : undefined;
          }
        })
      );
