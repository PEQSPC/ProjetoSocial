// src/pages/donations/Create.tsx
...
export default function DonationCreate() {
  ...
  // Ajusta estes paths se o teu backend usar outros nomes (ex.: /stockLots)
  const stockLotsPath = "/stock_lots";
  ...
  const onSave = async () => {
    ...
    // 2) Normalizar itemId nas linhas (resolver por EAN se necessário)
    const normalized = lines
      .map((l) => {
        ...
        return finalItemId ? { ...l, itemId: String(finalItemId) } : null;
      })
      .filter(Boolean) as (NewLine & { itemId: string })[];
    ...
    // 3) Agrupar por artigo para atualizar stock de uma vez por artigo
    const groupByItem = new Map<string, (NewLine & { itemId: string })[]>();
    for (const l of normalized) {
      const arr = groupByItem.get(l.itemId) ?? [];
      arr.push(l);
      groupByItem.set(l.itemId, arr);
    }

    // 4) Processar artigo a artigo
    for (const [itemId, itemLines] of groupByItem.entries()) {
      const itemName = items.find((i) => String(i.id) === itemId)?.name;

      // 4.1 – criar as linhas de doação e upsert de lotes
      for (const l of itemLines) {
        await api.post(donationLinesPath, {
          donationId: newId,
          itemId,
          itemName, // denormalizado
        ...
        if (l.lot) {
          // tenta encontrar lote igual (item + lot)
          const lotRes = await api.get<StockLotRow[]>(stockLotsPath, {
            params: { itemId, lot: l.lot },
          });
          const lotRows = Array.isArray(lotRes.data) ? lotRes.data : [];
          if (lotRows.length) {
            const existing = lotRows[0];
            await api.patch(`${stockLotsPath}/${existing.id}`, {
              quantity:
                Number(existing.quantity ?? 0) + Number(l.quantity ?? 0),
              remainingQty:
      ...
      // 4.2 – Recalcular o total do artigo
      // Plano A: somar remainingQty de todos os lotes
      // Plano B (fallback): somar o delta desta doação ao stockCurrent atual
      let newTotal: number | null = null;

      try {
        const lotsRes = await api.get<StockLotRow[]>(stockLotsPath, {
          params: { itemId },
        });
        const lots = Array.isArray(lotsRes.data) ? lotsRes.data : [];
        if (lots.length) {
          newTotal = lots.reduce<number>(
      ...
      if (newTotal === null) {
        const delta = itemLines.reduce<number>(
          (acc, l) => acc + Number(l.quantity ?? 0),
          0
        );
        try {
          const itemRes = await api.get(`${itemsPath}/${itemId}`);
          const current = Number(itemRes.data?.stockCurrent ?? 0);
          newTotal = current + delta;
        } catch {
          // se falhar, não escrevemos um valor potencialmente errado
          newTotal = undefined as unknown as number;
        }
      }

      if (typeof newTotal === "number" && !Number.isNaN(newTotal)) {
        await api.patch(`${itemsPath}/${itemId}`, { stockCurrent: newTotal });
      }
    }
